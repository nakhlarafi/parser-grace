...
ids = [list(range(3, 66)) + [1]] #active bug ids are 1, 3 - 65 for Lang
    ...
    for idx in ids[i]:
        ...
        try:
            ...
            for methodnamess in [1]:
                ...
                f = open('data_copy/FailingTests/%s/%s.txt' % (xss, idx))
                methods = open('data_copy/AllMethods/%s/%s.txt' % (xss, idx)).readlines()
                bmethods = []
                for x in methods:
                    lst = x.strip().split(":")
                    fname = lst[0] + ":" + lst[2]
                ...
                #print('----', tests)
                #failline = {}
                if not os.path.exists('data_copy/LineCoverage/%s/%s.txt' % (xss, idx)):
                    os.system('gzip -d %s' % 'data_copy/LineCoverage/%s/%s.txt.gz' % (xss, idx))
                f = open('data_copy/LineCoverage/%s/%s.txt' % (xss, idx), 'r')
                lines = f.readlines()
                methods = {}
                v = {}
                edge2 = []
                ...
                buggymethods = open('data_copy/BugMethod/%s/%s.txt' % (xss, idx)).readlines() 
                ...
                for x in tqdm(methods):
                    print(x)
                    fpath = 'buggy%d/'%idx + dirs + '/' + x.split(':')[0].split('$')[0].replace('.', '/') + ".java"
                ...
                edge6 = []
                edge7 = []
                if not os.path.exists('/data/zqh/f2p_id/line_f2p_id/line_type3_f2p/%s/%d.json'%(xss, idx)):
                    mus = {}
                else:
                    mus = json.load(open('/data/zqh/f2p_id/line_f2p_id/line_type3_f2p/%s/%d.json'%(xss, idx), 'r'))
                for k, x in enumerate(mus):
                    types1 =  0
                    types2 = 0
                    #lst = mus[x]['location'].split(":")
                    #mus[x]['location'] = lst[0] + ":" + lst[2]
                    idxxx = len(mutation)
                    #if mus[x]['mutator'] not in mutation:
                    #    mutation[mus[x]['mutator']] = len(mutation) 
                    mutation[idxxx] = mus[x]['mutator']
                    edge4.append((mus[x]['location'], idxxx))
                    failid = []         
                    for test in mus[x]['level-3']['failtests']:
                        testtype = test[0]
                        testid = test[1:]
                        if testtype == 'r':
                            edge5.append((int(testid), idxxx))
                        if testtype == 'f':
                            failid.append(int(testid))
                    fail2pass = list(set(range(len(tests))).intersection(set(failid)))
                    for x in fail2pass:
                        edge6.append((x, idxxx))
                mu = linenode
                linem = []
                if not os.path.exists('/data/zqh/FLocalization/mutation/stacktrace/%s/%d.json'%(xss, idx)):
                    st = {}
                else:
                    st = json.load(open('/data/zqh/FLocalization/mutation/stacktrace/%s/%d.json'%(xss, idx)))
                stl = {}
                for x in st:
                    for k, l in enumerate(st[x]):
                        if l in liness:
                            stl[liness[l]] = k + 1
                stll = []
                ...
                edge7 = []
                edge8 = []
                if not os.path.exists('/data/zqh/FLocalization/mutation/type1_f2p/%s/%d.json'%(xss, idx)):
                    callgraph = {}
                else:
                    callgraph = json.load(open('/data/zqh/FLocalization/mutation/callgraph_refine/%s/%d.json'%(xss, idx)))
                for x in callgraph:
                    if x not in methods:
                        continue
                    method1 = methods[x]
                    for l in callgraph[x]:
                        for callm in callgraph[x][l]:
                            line = x.split(":")[0] + ":" + str(l)
                            if callm not in methods:
                                continue
                            #print(line)
                            if line not in liness:
                                continue
                            #assert(0)
                            lineid = liness[line]
                            method2 = methods[callm]
                            edge6.append((method1, method2))
                            #edge8.append((liness['control1'], lineid))
                            #edge8.append((lineid, liness['control2']))'''
        ...
        open('%s%d.pkl' % (xss, idx), 'wb').write(pickle.dumps(res))#res.append(tmp)
print(err)
